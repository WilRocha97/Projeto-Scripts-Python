...
def lista_acordos(pagina):
    ...
    soup = BeautifulSoup(pagina.content, 'html.parser')
    tabela = soup.find('tbody', attrs={'id': 'formComposicaoPedido:j_id38:tbody_element'})
    if not tabela:
        return []
...
def download_guia(pagina, venc):
    ...
    soup = BeautifulSoup(pagina.content, 'html.parser')
    # 'formDetalharExtratoParcelamento:j_id94:tbody_element'
    tabela = soup.find('tbody', attrs={'id': 'formDetalharExtratoParcelamento:dataTableParcelamento:tbody_element'})
    for linha in tabela.findAll('tr'):
        colunas = linha.findAll('td')
...
def verifica_parcelamento(cnpj, session, venc):
    ...
    url_base = 'https://www2sp.dataprev.gov.br/ParcWebPrevInternet'

    url_inter = 'https://cav.receita.fazenda.gov.br/Servicos/ATBHE/DATAPREV/PARCWEBPREV/App.aspx'
    ...
    for acordo, link in acordos:
        ...
        if guia:
            ...
            nome = (
                f'{cnpj};IMP_FISCAL;{str_venc};Parcelamento Previdenciário RFB' +
                f'{list_venc[1]};{list_venc[2]};Parcelamento;INSS 2 - {acordo}.pdf'
            )

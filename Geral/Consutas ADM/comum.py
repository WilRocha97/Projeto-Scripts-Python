...
def new_session_fazenda_driver(window_principal, user, pwd, perfil, retorna_driver=False, options='padrão', timeout=90):
    ...
    status, driver = initialize_chrome(timeout, options)
    driver.get(url_home)
    ...
    if not resposta:
        ...
        try:
            ...
            if not resposta:
                padrao = re.compile(r'(Este usuário não possui uma senha definida)')
                resposta = padrao.search(str(soup))
                
                if not resposta:
                    padrao = re.compile(r'(Informe o login e senha corretamente.)')
                    resposta = padrao.search(str(soup))
                    ...
                    if not resposta:
                        ...
                        if not resposta:
                            ...
                            if not resposta:
                                padrao = re.compile(r'(Falha na resolução do CAPTCHA.)')
                                resposta = padrao.search(str(soup))
...
def indice(count, empresa, total_empresas, index, window_principal, tempos, tempo_execucao):
    ...
    if dias > 0 or horas > 0 or minutos > 0:
        ...
        # Retorna o tempo no formato "dias:horas:minutos:segundos"
        tempo_estimado_texto = f"  |  Tempo estimado:{dias_texto}{horas_texto}{minutos_texto}{segundos_texto}  |  Previsão de termino: {previsao_termino.strftime('%d/%m/%Y as %H:%M:%S')}"
...
def configura_dados(window_principal, continuar_rotina, codigo_20000, situacao_dados, planilha_dados, pasta_final_, andamentos, colunas_usadas=None, colunas_filtro=None, palavras_filtro=None, filtrar_celulas_em_branco=None):

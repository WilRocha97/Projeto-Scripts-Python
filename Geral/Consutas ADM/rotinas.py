# -*- coding: utf-8 -*-
import fitz, sys, shutil, re, os, traceback, requests.exceptions, pdfkit, selenium.common
...
def run_cndtni(window_principal, codigo_20000, situacao_dados, planilha_dados, pasta_final_, andamentos, tempos, tempo_execucao, continuar_rotina):
    ...
    def consulta_cndni(window_principal, driver, nome, cnpj, pasta_final_download_cndtni, pasta_final):
        ...
        while not comum.find_by_id('MainContent_txtDocumento', driver):
            ...
            timer += 1
            if timer > 60:
                print('Site demorou muito pra responder, tentando novamente...')
                window_principal['-Mensagens_2-'].update(f'O site demorou muito pra responder, tentando novamente...')
                window_principal.refresh()
                return driver, 'erro'
        ...
        while not comum.find_by_id('MainContent_lnkImprimirCertidaoBotao1', driver):
            ...
            contador += 1
            if contador > 15:
                window_principal['-Mensagens_2-'].update(f'Erro ao consultar CNDNI, tentando novamente...')
                window_principal.refresh()
        ...
        while True:
            ...
            if re.compile(r"Server Error in '/CertidaoNegativaDeb' Application.").search(driver.page_source):
                ...
                return driver, 'erro'
            try:
                os.makedirs(pasta_final_download_cndtni, exist_ok=True)
                while os.listdir(pasta_final_download_cndtni) == []:
                    print('>>> Tentando baixar o documento...')
                    driver.execute_script("window.scrollBy(0,200)")
                    driver.find_element(by=By.ID, value='MainContent_lnkImprimirCertidaoBotao1').click()
                    time.sleep(3)
                
                resultado = renomeia_cndni(window_principal, nome, cnpj, pasta_final_download_cndtni, pasta_final)
                if resultado:
                    break
                cr = open(controle_rotinas, 'r', encoding='utf-8').read()
                if cr == 'ENCERRAR':
    ...
    def renomeia_cndni(window_principal, nome, cnpj, pasta_final_download_cndtni, pasta_final):
        ...
        for cndni in os.listdir(pasta_final_download_cndtni):
            arq = os.path.join(pasta_final_download_cndtni, cndni)
            ...
            for page in doc:
                ...
                if debitos == 'sim':
                    ...
                    if debitos_lista != '':
                        ...
                        shutil.move(arq, os.path.join(pasta_debito, f'{nome[:25]} - {cnpj} - CNDNI Débitos{debitos_lista}.pdf'))
                        print('❗ Com Débitos')
                        return 'Com débitos'
...
def run_debitos_municipais_jundiai(window_principal, codigo_20000, situacao_dados, planilha_dados, pasta_final_, andamentos, tempos, tempo_execucao, continuar_rotina):
    def login_jundiai(window_principal, cnpj, insc_muni, pasta_final):
        ...
        if not response:
            return driver, 'encerrar', '❌ Erro no captcha'
...
def run_debitos_municipais_valinhos(window_principal, codigo_20000, situacao_dados, planilha_dados, pasta_final_, andamentos, tempos, tempo_execucao, continuar_rotina):
    ...
    def format_data(content):
        ...
        with open('Assets\style_debitos_valinhos.css', 'r') as f:
            css = f.read()
    ...
    def consulta_valinhos(driver, cnpj, insc_muni, nome, andamentos, pasta_final):
        ...
        if str_html:
            ...
            with open(os.path.join(pasta_final, 'Arquivos', nome_arq + '.pdf'), 'wb') as pdf:
...
def run_debitos_municipais_vinhedo(window_principal, codigo_20000, situacao_dados, planilha_dados, pasta_final_, andamentos, tempos, tempo_execucao, continuar_rotina):
        ...
        try:
            ...
            while 'dados-contribuinte-inscricao">0000000000' not in driver.page_source:
                ...
                if comum.find_by_path('/html/body/div[1]/div[5]/div[2]/div[1]/div/ul/li/span', driver):
                    ...
                    if situacao == 'Letras de segurança inválidas':
                        driver.close()
                        return False, 'recomeçar'
                    if situacao == 'Por favor, informe o(a) Inscrição Cadastral':
                        situacao = 'Inscrição Cadastral inválida'
                    situacao_print = f'❌ {situacao}'
            ...
            situacao = salvar_guia_vinhedo(window_principal, driver, cnpj, pasta_final_certidao)
            cr = open(controle_rotinas, 'r', encoding='utf-8').read()
            if cr == 'ENCERRAR':
                try:
                    driver.close()
    ...
    def salvar_guia_vinhedo(window_principal, driver, cnpj, pasta_final_certidao):
        ...
        while True:
            try:
                shutil.move(os.path.join(pasta_final_certidao, 'impressao.pdf'), os.path.join(pasta_final_certidao, f'{cnpj} Certidão Negativa de Débitos Municipais Vinhedo.pdf'))
                break
            except:
                pass
        driver.close()
...
def run_debitos_divida_ativa_uniao(window_principal, codigo_20000, situacao_dados, planilha_dados, pasta_final_, andamentos, tempos, tempo_execucao, continuar_rotina):
    ...
    def download_divida(driver, divida, descricao, tipo, processo, inscricao, andamentos, pasta_final_download_dau, pasta_final_dividas):
        ...
        os.makedirs(pasta_final_download_dau, exist_ok=True)
        
        # limpa a pasta de download para não ter conflito com novos arquivos
        for arquivo in os.listdir(pasta_final_download_dau):
            os.remove(os.path.join(pasta_final_download_dau, arquivo))
        ...
        contador_3 = 0
        contador_4 = 0
        while os.listdir(pasta_final_download_dau) == []:
            if contador_3 > 10:
                while not click(driver, divida):
                    time.sleep(5)
                contador_4 += 1
        ...
        for arquivo in os.listdir(pasta_final_download_dau):
            ...
            while re.compile(r'.tmp').search(arquivo):
                try:
                    ...
                    pass
                for arq in os.listdir(pasta_final_download_dau):
                    arquivo = arq
            ...
            for arq in os.listdir(pasta_final_download_dau):
                resultado = converte_html_pdf(pasta_final_download_dau, pasta_final_dividas, arq, descricao, tipo, processo, inscricao, andamentos)
                if resultado == 'erro':
    ...
    def consulta_lista(window_principal, driver, andamentos, pasta_final_download_dau, pasta_final_dividas):
        ...
        for pagina in range(int(paginas) + 1):
            ...
            for divida in lista_divida:
                ...
                print('>>> Buscando dívida...')
                if cpf_cnpj_anterior != 0:
                    print(f'{cpf_cnpj}  |  {processo}  |  {inscricao}')
                ...
                tempos, tempo_execucao = comum.indice(contador, divida, total_de_dividas, index, window_principal, tempos, tempo_execucao)
                ...
                while download:
                    ...
                    driver, erro, download = download_divida(driver, divida, descricao, tipo, processo, inscricao, andamentos, pasta_final_download_dau, pasta_final_dividas)
                    if erro == 'Finalizado':
                        return driver
    ...
    while True:
        ...
        window_principal['-Mensagens-'].update('SIEG IRIS demorou muito pra responder, tentando novamente')
        window_principal.refresh()
...
def run_debitos_divida_ativa(window_principal, codigo_20000, situacao_dados, planilha_dados, pasta_final_, andamentos, tempos, tempo_execucao, continuar_rotina):
    ...
    def salva_pagina(pagina, cnpj, nome, andamentos, pasta_final, compl=''):
        ...
        with open('Assets\\style_divida_ativa.css', 'r') as f:
            css = f.read()
        ...
        nome_pdf = os.path.join(pasta_documentos, f'{cnpj};INF_FISC_REAL;Procuradoria Federal{debito.replace(" / ", " ").replace("/", " ")}.pdf')
        # cria o PDF a partir do HTML usando a função "pisa"
        with open(nome_pdf, 'wb') as pdf:
            # pisa.showLogging()
            pisa.CreatePDF(str(html), pdf)
        print('✔ Arquivo salvo')
        comum.escreve_relatorio_xlsx({'CNPJ': cnpj, 'NOME': nome, 'RESULTADO': 'Empresa com débitos', 'COMPLEMENTO': compl},
                                    nome=andamentos, local=pasta_final)
    ...
    def consulta_debito(window_principal, s, nome_botao_consulta, empresa, andamentos, pasta_final):
        ...
        try:
            ...
            comum.escreve_relatorio_xlsx({'CNPJ': cnpj_formatado, 'NOME': nome, 'RESULTADO': e, 'COMPLEMENTO': compl},
                                         nome=andamentos, local=pasta_final)
            print(e)
            print(soup)
            s.close()
        ...
        pagina = s.post(url, info, timeout=60)
        ...
        if not debitos:
            print('✔ Sem débitos')
            comum.escreve_relatorio_xlsx({'CNPJ': cnpj_formatado, 'NOME': nome, 'RESULTADO': 'Empresa sem debitos', 'COMPLEMENTO': ''},
                                         nome=andamentos, local=pasta_final)
        # se tiver débitos pega quantos débitos tem para montar a PDF
            ...
            for index, linha in enumerate(linhas):
                ...
                salva_pagina(pagina, cnpj_formatado, nome, andamentos, pasta_final, linha.text)
    ...
    while True:
        ...
        if nome_botao_consulta == 'erro_captcha':
            window_principal['-Mensagens_2-'].update('Erro ao iniciar a sessão, tentando novamente')
            window_principal.refresh()
            print('Erro ao iniciar a sessão, tentando novamente')
        
        elif nome_botao_consulta == 'erro_site':
            window_principal['-Mensagens_2-'].update('O site demorou muito pra responder, tentando novamente')
            window_principal.refresh()
        ...
        while True:
            ...
            with Session() as s:
                ...
                try:
                    ...
                    window_principal['-Mensagens_2-'].update('O site demorou muito para responder, tentando novamente...')
                    window_principal.refresh()
                    pass
...
def executa_rotina(window_principal, codigo_20000, situacao_dados, planilha_dados, pasta_final, rotina, continuar_rotina):
    ...
    if resultado:
        ...
        cr = open(controle_rotinas, 'r', encoding='utf-8').read()
        if cr == 'ENCERRAR':
            alert(f'❗ Rotina "{rotina}" encerrada')
        try:

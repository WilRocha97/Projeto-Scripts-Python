...
def login_email(driver):
    ...
    _wait_img('menu_classificacao.png', conf=0.9, timeout=-1)
    _click_img('menu_classificacao.png', conf=0.9)
    _wait_img('crescente.png', conf=0.9, timeout=-1)
    time.sleep(0.5)
...
def captura_dados_email(driver):
    ...
    if _find_img('anexos.png', conf=0.9) or _find_img('anexos_2.png', conf=0.9):
        ...
        _wait_img('baixar_anexo.png', conf=0.9, timeout=-1)
        #_click_position_img('baixar_anexo.png', '+', pixels_y=50, conf=0.9)
        _click_img('anexo_pdf.png', conf=0.99)
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        # pega cnpj da empresa que vai receber a mensagem
                        cnpj = re.compile(r'CNPJ:.+(\d\d\d\d\d\d\d\d\d\d\d)').search(driver.page_source).group(1)
                        cnpj_limpo = cnpj.replace('.', '').replace('/', '').replace('-', '')
                        print(cnpj)
                        print(cnpj_limpo)
                    except:
...
def enviar_sem_anexo(numero, link_mensagem, titulo, vencimento):
    ...
    _wait_img('nova_conversa.png', conf=0.9, timeout=-1)
    _click_img('nova_conversa.png', conf=0.9)
    time.sleep(1)
    _wait_img('nova_conversa_titulo.png', conf=0.9, timeout=-1)
    p.write(numero)
    time.sleep(2)
    ...
    if _find_img('sem_contato.png', conf=0.9):
        ...
        """p.hotkey('ctrl', 'w')
        time.sleep(1)"""
        return 'Nenhum contato encontrado, número inválido ou não possuí WhatsApp'
    ...
    _wait_img('anexar.png', conf=0.9, timeout=-1)
    time.sleep(1)
    
    resultado, numero_copiado = verifica_numero(numero)
    if not resultado:
        return f'Número do cliente: {numero_copiado} não confere com o número capturado da planilha'
    
    while True:
        try:
            print('Tentando copiar a mensagem do e-mail')
    ...
    time.sleep(1)
    _click_img('enviar.png', conf=0.9, timeout=-1)
    # time.sleep(3)
...
def enviar_anexo(numero, anexo, corpo_email):
    ...
    resultado, numero_copiado = verifica_numero(numero)
    if not resultado:
        return f'Número {numero_copiado} não confere com o pesquisado'
    
    _wait_img('anexar.png', conf=0.9, timeout=-1)
    _click_img('anexar.png', conf=0.9)
    time.sleep(1)
    _wait_img('documento.png', conf=0.9, timeout=-1)
    ...
    time.sleep(1)
    p.press('enter')
    _wait_img('digitar_mensagem.png', conf=0.9, timeout=-1)
    time.sleep(1)
    while True:
        try:
            pyperclip.copy(mensagem)
    ...
    time.sleep(1)
    _click_img('enviar_verde.png', conf=0.9, timeout=-1)
    # time.sleep(3)
...
def verifica_numero(numero):
    _click_position_img('nova_conversa.png', '+', pixels_x=250, conf=0.9)
    _wait_img('dados_contato.png', conf=0.9, timeout=-1)
    time.sleep(1)
    numero_copiado = ''
    
    if _find_img('conta_comercial.png', conf=0.9) or _find_img('dados_contato_2.png', conf=0.9):
        while not _find_img('apagar_conversa.png', conf=0.9):
            p.click(1576, 834)
        _click_position_img('numero_contato_2.png', '+', pixels_x=50, conf=0.98)
        p.click(clicks=3)
        time.sleep(0.5)
    else:
        if _find_img('numero_contato_3.png', conf=0.9):
            _click_position_img('numero_contato_3.png', '+', pixels_x=120, conf=0.96)
        if _find_img('numero_contato.png', conf=0.9):
            _click_position_img('numero_contato.png', '+', pixels_x=100, conf=0.96)
            
        p.click(clicks=3)
        time.sleep(0.5)
        
    """for i in :[(286, 50), (290, 105), (312, 128)]
    if _find_img('conta_comercial.png', conf=0.9) or _find_img('dados_contato_2.png', conf=0.9):
        if i[1]:
            continue
        while not _find_img('apagar_conversa.png', conf=0.9):
            p.click(1576,834)
        _click_position_img('dados_contato_2.png', '+', pixels_y=i[1], conf=0.9)
        p.click(clicks=3)
        time.sleep(0.5)
        ...
        if i[0]:
            continue
        _click_position_img('dados_contato.png', '+', pixels_y=i[0], conf=0.9)
        p.click(clicks=3)
        time.sleep(0.5)"""
    ...
    while True:
        try:
            ...
            pass
    numero_copiado = numero_copiado.replace('+55', '').replace('-', '').replace(' ', '').strip()
    if str(numero) == str(numero_copiado):
        _click_position_img('dados_contato.png', '-', pixels_x=135, conf=0.9)
        time.sleep(1)
        print('Número confere')
        return True, numero_copiado
        
    print('Número não confere')
    _click_position_img('dados_contato.png', '-', pixels_x=135, conf=0.9)
    time.sleep(1)
    return False, numero_copiado
        ...
        if titulo[:3] == 'Re:':
            ...
            # se tiver como enviar e encontrar o número na planilha
                ...
                while not _find_img('nova_conversa.png', conf=0.9):
                    if _find_img('bluepex.png', conf=0.95):
                        _click_img('bluepex.png', conf=0.95)
                        time.sleep(0.5)
                        p.hotkey('alt', 'f4')
                        time.sleep(0.5)

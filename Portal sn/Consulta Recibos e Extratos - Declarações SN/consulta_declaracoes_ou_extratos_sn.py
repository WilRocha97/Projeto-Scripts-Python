...
def consulta_extratos(cnpj, mes, ano, session):
    ...
    extratos = soup.findAll('a', attrs={'data-content': 'Imprimir Declaração'})

    download = (0, 0)
    for extrato in extratos:
        link = extrato.get('href', '')
        if not link:
            continue
    ...
    if res.headers.get('content-type', '') != 'application/pdf':
        print('❌ Erro - Não pode fazer download do extrato')
        return 'Erro - Não pode fazer download do extrato'

    # salva o PDF do extrato
    nome = f'declaração_sn {cnpj} {mes}{ano}.pdf'
    _download_file(nome, res, pasta='execucao/Declarações - Extratos')

    print('✔ Extrato disponível')
    return 'Extrato disponível'
            ...
            while captcha == 'erro':
                ...
                if session == 'Erro Login - Caracteres anti-robô inválidos. Tente novamente.':
                    ...
                    if isinstance(session, Session):
                        text = consulta_extratos(cnpj, *comp, session)
                        driver.quit()
                    else:
                        text = session
                        driver.quit()
                    ...
                    _escreve_relatorio_csv(f'{cnpj};{text}', nome='Declarações - Extratos SN')
    return True

...
def login(options, cnpj, insc_muni):
    ...
    if captcha is not None:
        ...
        if find_by_id('tdMsg', driver):
            ...
            _escreve_relatorio_csv(';'.join([cnpj, 'Erro no login']))
            print('❌ Erro no login')
        else:
            while not find_by_id('tdTitulo', driver):
                time.sleep(1)
        ...
        _escreve_relatorio_csv(';'.join([cnpj, 'Erro no captcha']))
...
def run():
    ...
    for count, empresa in enumerate(empresas[index:], start=1):
        ...
        if not login(options, cnpj, insc_muni):
            break

...
def login_gov(cpf, senha):
    ...
    p.moveTo(5, 5)
    while not _find_img('botao_gov.png', pasta='imgs_c', conf=0.95):
        if _find_img('sair_fgts.png', pasta='imgs_c', conf=0.95):
            _click_img('sair_fgts.png', pasta='imgs_c', conf=0.95)
        
        if _find_img('sair.png', pasta='imgs_c', conf=0.95):
            _click_img('sair.png', pasta='imgs_c', conf=0.95)
        
        if _find_img('campo_cpf_selecionado.png', pasta='imgs_c', conf=0.95):
            break
        
        if _find_img('site_morreu.png', pasta='imgs_c', conf=0.95):
            p.hotkey('ctrl', 'w')
            time.sleep(1)
            p.hotkey('ctrl', 'w')
    ...
    while not _find_img('campo_cpf_selecionado.png', pasta='imgs_c', conf=0.95):
        if _find_img('sair.png', pasta='imgs_c', conf=0.95):
            _click_img('sair.png', pasta='imgs_c', conf=0.95)
        # clica no botão do gov.br
        p.moveTo(5, 5)
        _click_img('botao_gov.png', pasta='imgs_c', conf=0.95, timeout=1)
        if _find_img('cadastro_incompleto.png', pasta='imgs_c', conf=0.95):
            print('>>> Fechando login anterior')
            p.hotkey('ctrl', 'w')
            time.sleep(1)
            _acessar_site_chrome('https://www.gov.br/pt-br')
            print('>>> Aguardando Gov.br')
            while not _find_img('tela_inicial_gov.png', pasta='imgs_c', conf=0.95):
                time.sleep(1)
            
            print('>>> Fechando login')
            if _find_img('entrar_govbr.png', pasta='imgs_c', conf=0.95):
                _click_img('entrar_govbr.png', pasta='imgs_c', conf=0.95)
            
            while not _find_img('tela_inicial_gov.png', pasta='imgs_c', conf=0.95):
                if _find_img('ja_fechou_usuario.png', pasta='imgs_c', conf=0.95):
            ...
            while not _find_img('sair_da_conta.png', pasta='imgs_c', conf=0.95):
                p.press('pgDn')
            
            _click_img('sair_da_conta.png', pasta='imgs_c', conf=0.95)
            time.sleep(1)
            _acessar_site_chrome('https://meu.inss.gov.br/#/login')
            return 'recomeçar'
        
        if _find_img('alerta_acessos.png', pasta='imgs_c', conf=0.9):
            # clica no campo do cpf
            _click_img('alerta_acessos.png', pasta='imgs_c', conf=0.9)
            time.sleep(0.5)
            p.press('esc')
        
        if _find_img('campo_cpf.png', pasta='imgs_c', conf=0.9):
            # clica no campo do cpf
            _click_img('campo_cpf.png', pasta='imgs_c', conf=0.9)
    ...
    while _find_img('campo_cpf_selecionado.png', pasta='imgs_c', conf=0.95):
        p.write(cpf)
        time.sleep(1)
    p.press('enter')
    ...
    while not _find_img('campo_senha_selecionado.png', pasta='imgs_c', conf=0.95):
        ...
        for erro in erros:
            if _find_img(erro[0], pasta='imgs_c', conf=0.95):
                p.moveTo(5, 5)
                if erro[2]:
                    _click_img(erro[2], pasta='imgs_c', conf=0.95)
                if erro[3]:
                    p.press('enter')
                if erro[4]:
        ...
        timer_2 = 0
        while _find_img('captcha.png', pasta='imgs_c', conf=0.95):
            time.sleep(1)
            timer_2 += 1
    ...
    while _find_img('campo_senha_selecionado.png', pasta='imgs_c', conf=0.95):
        p.write(senha)
        time.sleep(1)
    p.press('enter')
    ...
    # aguarda o site carregar
    while not _find_img('servicos.png', pasta='imgs_c', conf=0.9):
        ...
        for erro_2 in erros_2:
            if _find_img(erro_2[0], pasta='imgs_c', conf=0.95):
                p.moveTo(5, 5)
                if erro_2[2]:
                    _click_img(erro_2[2], pasta='imgs_c', conf=0.95)
